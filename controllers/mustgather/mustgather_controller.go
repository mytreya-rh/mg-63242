...
func (r *MustGatherReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		if instance.Spec.UploadTarget != nil && instance.Spec.UploadTarget.SFTP != nil && instance.Spec.UploadTarget.SFTP.CaseManagementAccountSecretRef.Name != "" {
			...
			if err != nil {
				if errors.IsNotFound(err) {
					userErr := fmt.Errorf("secret %s not found in namespace %s: Please create the secret referenced by caseManagementAccountSecretRef", secretName, instance.Namespace)
					log.Error(userErr, "caseManagementAccountSecretRef secret not found")
					return r.ManageError(ctx, instance, userErr)
				}
