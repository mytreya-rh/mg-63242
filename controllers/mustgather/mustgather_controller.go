...
func (r *MustGatherReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		if instance.Spec.UploadTarget != nil && instance.Spec.UploadTarget.SFTP != nil && instance.Spec.UploadTarget.SFTP.CaseManagementAccountSecretRef.Name != "" {
			...
			if err != nil {
				if errors.IsNotFound(err) {
					errMsg := fmt.Errorf("secret %s not found in namespace %s: Please create the secret referenced by caseManagementAccountSecretRef", secretName, instance.Namespace)
					log.Error(err, fmt.Sprintf("the secret was not found: %s", errMsg.Error()))
					return r.ManageError(ctx, instance, errMsg)
				}
